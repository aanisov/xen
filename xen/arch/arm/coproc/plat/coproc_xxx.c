...
static int vcoproc_xxx_read(struct vcpu *v, mmio_info_t *info, register_t *r, void *priv)
{
	...
	struct vcoproc_instance *vcoproc_xxx = coproc_xxx_get_vcoproc(v->domain, coproc_xxx);
...
static struct vcoproc_instance *vcoproc_xxx_vcoproc_init(struct domain *d, struct coproc_device *coproc_xxx)
{
...
static bool_t coproc_xxx_vcoproc_is_created(struct domain *d, struct coproc_device *coproc_xxx)
{
