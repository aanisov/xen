    ...
    if ( likely(!rc) )
    {
        ...
        if ( need_iommu(p2m->domain) && !iommu_use_hap_pt(p2m->domain) )
        {
            if ( !mfn_eq(orig_smfn, INVALID_MFN) )
            {
                ...
                /* TODO Do we need to unmap if failed? */
...
int relinquish_p2m_mapping(struct domain *d)
{
    ...
    /*
     * TODO Here we need to remove mapping if:
     * rc==0
     * need_iommu(p2m->domain)
     * !iommu_use_hap_pt(p2m->domain)
     */
...
#undef ACCESS
    ...
    /*
     * TODO Here we need to apply mapping with new permissions if:
     * rc==0
     * need_iommu(p2m->domain)
     * !iommu_use_hap_pt(p2m->domain)
     */
