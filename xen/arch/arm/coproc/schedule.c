...
static void vcoproc_scheduler_set_current(const struct vcoproc_scheduler *sched, struct vcoproc_instance *vcoproc)
{
...
void vcoproc_sheduler_vcoproc_wake(struct vcoproc_scheduler *sched, struct vcoproc_instance *vcoproc)
{
	struct vcoproc_schedule_data *sched_data = sched->sched_priv;
	unsigned long flags;
...
void vcoproc_sheduler_vcoproc_sleep(struct vcoproc_scheduler *sched, struct vcoproc_instance *vcoproc)
{
	struct vcoproc_schedule_data *sched_data = sched->sched_priv;
	unsigned long flags;
	int reschedule = 0;
	...
	spin_lock_irqsave(&sched_data->schedule_lock, flags);
	if ( vcoproc->state != VCOPROC_WAITING && vcoproc->state != VCOPROC_RUNNING )
	{
		spin_unlock_irqrestore(&sched_data->schedule_lock, flags);
		return;
	}
...
void vcoproc_sheduler_vcoproc_yield(struct vcoproc_scheduler *sched, struct vcoproc_instance *vcoproc)
{
	struct vcoproc_schedule_data *sched_data = sched->sched_priv;
	unsigned long flags;
