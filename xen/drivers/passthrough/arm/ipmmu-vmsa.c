...
/*
 * We can either flush page table by request from the P2M common code or use
 * our allocator decisions about when it is time to flush.
 */
/*#define CONFIG_USE_P2M_DRIVEN_FLUSH*/
 ...
 * configuration of the IOMMU) but, on Xen, we also have to store the
...
struct ipmmu_vmsa_xen_device {
	...
	/* We also have to store arch specific information like Linux does for IPMMU */
...
#define dev_iommu_group(dev) (dev_iommu(dev)->group)
#define dev_iommu_archdata(dev) (dev_iommu(dev)->archdata)

/* Xen: Dummy iommu_group management */
...
static __maybe_unused void iommu_group_remove_device(struct device *dev)
{
	struct iommu_group *group = dev_iommu_group(dev);

	if (group) {
		dev_iommu_group(dev) = NULL;
		atomic_dec(&group->ref);
	}
}
			     ...
			     u32 data)
{
	...
	/* ASSERT(spin_is_locked(&xen_domain->lock));*/
...
static void ipmmu_tlb_sync(struct ipmmu_vmsa_domain *domain)
{
	...
	while (ipmmu_ctx_read(domain, IMCTR) & IMCTR_FLUSH) {
		...
		if (++count == TLB_LOOP_TIMEOUT) {
			/* Xen: It would be correct to use domain->root->dev here... */
...
/* Xen: */
#ifndef CONFIG_USE_P2M_DRIVEN_FLUSH
...
static int ipmmu_domain_init_context(struct ipmmu_vmsa_domain *domain)
{
	...
	/* Xen: */
	dev_notice(domain->root->dev, "Set IPMMU context %u (pgd 0x%"PRIpaddr") for domain %d\n",
			domain->context_id, ttbr, domain->d->domain_id);
...
static struct iommu_group *ipmmu_find_group(struct device *dev)
{
	...
	if (!sibling || IS_ERR_OR_NULL(group))
...
static void ipmmu_destroy_platform_device(struct device *dev)
{
	struct ipmmu_vmsa_archdata *archdata = dev_iommu_archdata(dev);
...
/* Add check for xen_domain */
static int __must_check ipmmu_vmsa_iotlb_flush_all(struct domain *d)
{
#ifdef CONFIG_USE_P2M_DRIVEN_FLUSH
	struct ipmmu_vmsa_xen_domain *xen_domain = dom_iommu(d)->arch.priv;
	struct ipmmu_vmsa_domain *domain = to_vmsa_domain(xen_domain->base_context);
...
static int ipmmu_vmsa_add_dev(struct device *dev)
{
	struct ipmmu_vmsa_archdata *archdata;
	struct iommu_group *group;
	int ret;
	...
	if (dev_iommu_archdata(dev)) {
		dev_err(dev, "already assigned to IPMMU driver\n");
		return -EINVAL;
	}
	...
	archdata = dev_iommu_archdata(dev);
	spin_lock(&ipmmu_slave_devices_lock);
	list_add(&archdata->list, &ipmmu_slave_devices);
	spin_unlock(&ipmmu_slave_devices_lock);
...
out:
	ipmmu_destroy_platform_device(dev);
...
static int ipmmu_vmsa_domain_init(struct domain *d)
{
	...
	if (!root) {
		printk("Unable to locate root IPMMU\n");
		ret = -EAGAIN;
		goto out;
	}
	domain->root = root;
	...
	ret = ipmmu_domain_init_context(domain);
	if (ret < 0) {
		dev_err(root->dev, "Unable to initialize IPMMU context\n");
...
static void ipmmu_vmsa_domain_teardown(struct domain *d)
{
	...
	io_domain = xen_domain->base_context;
	if (io_domain)
		ipmmu_vmsa_destroy_domain(io_domain);
...
size_t ipmmu_unmap(struct iommu_domain *io_domain, unsigned long iova, size_t size)
{
	...
	/*printk("unmap this: iova 0x%lx size 0x%zx\n", iova, size);*/
	...
	while (unmapped < size) {
		...
		/*printk("unmapped: iova 0x%lx size 0x%zx\n", iova, unmapped_page);*/
		...
		phys_addr_t paddr, size_t size, int prot)
{
	...
	if (!IS_ALIGNED(iova | paddr | size, min_pagesz)) {
		printk("unaligned: iova 0x%lx pa 0x%lx size 0x%zx min_pagesz 0x%x\n",
		       iova, paddr, size, min_pagesz);
		return -EINVAL;
	}

	/*printk("map: iova 0x%lx pa 0x%lx size 0x%zx\n", iova, paddr, size);*/

	while (size) {
		size_t pgsize = ipmmu_pgsize(io_domain, iova | paddr, size);

		/*printk("mapping: iova 0x%lx pa 0x%lx pgsize 0x%zx\n",
				iova, paddr, pgsize);*/
		...
		if (ret == -EEXIST) {
			...
			if (exist_paddr == paddr) {
				/*printk("mapping already exist: iova 0x%lx pa 0x%lx pgsize 0x%zx\n",
						iova, paddr, pgsize);*/
				ret = 0;
			} else if (exist_paddr) {
				printk("create new mapping: iova 0x%lx pa 0x%lx pgsize 0x%zx\n",
						iova, paddr, pgsize);
...
#if 0
	/* TODO Sanity check - remove it later */
	if (!ret) {
		unsigned long i;
		...
		for (i = 0; i < page_count; i ++) {
			...
			if (!paddr || paddr != pfn_to_paddr(mfn + i)) {
				printk("d%d: IOMMU after mapping check gfn %#lx to mfn %#lx failed (iter %lu)\n",
					d->domain_id, gfn + i, mfn + i, i);
				ret = -1;
				break;
			}
		}
	...
	if (!ret) {
		...
		for (i = 0; i < page_count; i ++) {
			...
			if (paddr || paddr == pfn_to_paddr(gfn + i)) {
				printk("d%d: IOMMU after unmapping check gfn %#lx failed (iter %lu)\n",
					d->domain_id, gfn + i, i);
				ret = -1;
				break;
			}
