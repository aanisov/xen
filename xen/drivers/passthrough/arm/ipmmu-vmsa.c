 ...
 *
 * Based on Linux's IPMMU-VMSA driver:
...
{
    spinlock_t lock;
    /* One or more Cache IPMMU domains associated to this Xen domain */
    struct list_head cache_domains;
    /* Root IPMMU domain associated to this Xen domain */
...
struct ipmmu_vmsa_xen_device
{
    ...
    /* Master device's IPMMU arch specific information */
    struct ipmmu_vmsa_fwspec *fwspec;
};
...
#define dev_iommu_fwspec(dev) (dev_archdata(dev)->fwspec)
...
struct ipmmu_vmsa_domain
{
    ...
    /* Xen domain associated to this IPMMU domain */
    struct domain *d;
    ...
    /*
     * The number of master devices which are attached to this IPMMU domain.
     * Master devices behind the same IPMMU device are grouped together by
     * putting into the same IPMMU domain.
     * Only when the ref.counter reaches 0 this IPMMU domain can be destroyed.
     */
    atomic_t ref;
...
static int ipmmu_attach_device(struct ipmmu_vmsa_domain *domain,
                               struct device *dev)
{
    struct ipmmu_vmsa_fwspec *fwspec = dev_iommu_fwspec(dev);
    struct ipmmu_vmsa_device *mmu = fwspec->mmu;
    unsigned int i;
    ...
    for ( i = 0; i < fwspec->num_utlbs; ++i )
        ipmmu_utlb_enable(domain, fwspec->utlbs[i]);
                                ...
                                struct device *dev)
{
    ...
    for ( i = 0; i < fwspec->num_utlbs; ++i )
        ipmmu_utlb_disable(domain, fwspec->utlbs[i]);
}
...
static int ipmmu_init_master(struct device *dev)
{
    ...
    fwspec = xzalloc(struct ipmmu_vmsa_fwspec);
    if ( !fwspec )
    {
        ret = -ENOMEM;
        goto error;
    ...
    fwspec->mmu = mmu;
    fwspec->utlbs = utlbs;
    fwspec->num_utlbs = num_utlbs;
    dev_iommu_fwspec(dev) = fwspec;
...
static void ipmmu_populate_masters(struct ipmmu_vmsa_device *mmu)
{
 ...
 * until the Root IPMMU device has been registered for sure.
 *
                                                        ...
                                                        struct device *dev)
{
    ...
    /*
     * Loop through all Cache IPMMU domains associated to this Xen domain
...
static struct ipmmu_vmsa_domain *ipmmu_alloc_root_domain(struct domain *d)
{
    ...
    if ( !root )
    {
        printk(XENLOG_ERR "d%d: Unable to locate Root IPMMU\n", d->domain_id);
...
{
    struct ipmmu_vmsa_xen_domain *xen_domain = dom_iommu(d)->arch.priv;
    struct ipmmu_vmsa_domain *domain = dev_iommu_domain(dev);
...
static int ipmmu_iommu_domain_init(struct domain *d)
{
    ...
    /*
     * The Root IPMMU domain will be created on demand only, when the first
     * master device being attached to this Xen domain.
    */
...
static void ipmmu_iommu_domain_teardown(struct domain *d)
{
    ...
    /*
     * We assume that all master devices have been already detached from
...
static __init bool ipmmu_stage2_supported(void)
{
    ...
    if ( !np )
    {
        printk(XENLOG_ERR "Failed to find PRR node\n");
        return false;
    }

    if ( dt_device_get_address(np, 0, &addr, &size) )
    {
        printk(XENLOG_ERR "Failed to get PRR MMIO\n");
        return false;
    }
    ...
    if ( !base )
    {
        printk(XENLOG_ERR "Failed to ioremap PRR MMIO\n");
        return false;
    }
...
static __init int ipmmu_init(struct dt_device_node *node, const void *data)
{
    ...
    /*
     * Even if the IPMMU device can't be initialized, we don't want to give
     * the device to dom0.
     */
