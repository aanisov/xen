                    ...
                    unsigned long page_count, unsigned int flags)
{
    ...
    {
        /*printk("d%d: IOMMU try to map chunk gfn %#lx to mfn %#lx page count %lu\n",
               d->domain_id, gfn, mfn, page_count);*/
        rc = hd->platform_ops->map_pages(d, gfn, mfn, page_count, flags);
    }
    else if ( hd->platform_ops->map_page )
    {
        /*printk("d%d: IOMMU try to map single gfn %#lx to mfn %#lx page count %lu\n",
               d->domain_id, gfn, mfn, page_count);*/
                      ...
                      unsigned long page_count)
{
    ...
    {
        /*printk("d%d: IOMMU try to unmap chunk gfn %#lx page count %lu\n",
               d->domain_id, gfn, page_count);*/
        rc = hd->platform_ops->unmap_pages(d, gfn, page_count);
    }
    else if ( hd->platform_ops->unmap_page )
    {
        /*printk("d%d: IOMMU try to unmap single gfn %#lx page count %lu\n",
               d->domain_id, gfn, page_count);*/
