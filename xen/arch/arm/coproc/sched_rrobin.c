...
static inline void runq_insert_tail(const struct vcoproc_scheduler *ops, struct rr_vcoproc *svc)
{
	struct rr_private *priv = rr_priv(ops);
...
static inline void runq_insert_head(const struct vcoproc_scheduler *ops, struct rr_vcoproc *svc)
{
	struct rr_private *priv = rr_priv(ops);
...
static void rr_yield(const struct vcoproc_scheduler *ops, struct vcoproc_instance *vcoproc)
{
	/* nothing to do */
}
